...
namespace Plugin::Internal::Functors
{
	...
	ILoadDelayFunctor::ILoadDelayFunctor(RE::BSFixedString a_saveName, bool a_isSilent, RE::BSScript::StackID a_stackId)
	...
	ILoadDelayFunctor::ILoadDelayFunctor(F4SE::SerializationTag a_serializerTag)
		: ISharedDelayFunctor(a_serializerTag)
	{
	}

	ILoadDelayFunctor::~ILoadDelayFunctor() noexcept = default;

	bool ILoadDelayFunctor::ShouldReschedule(std::int32_t& a_outDelayMS)
	{
		a_outDelayMS = F4SE::IDelayFunctor::EMPTY_DELAY;
		return false;
	}

	bool ILoadDelayFunctor::ShouldResumeStack(RE::BSScript::StackID& a_outStackId)
	{
	...
	bool ILoadDelayFunctor::Load(const F4SE::SerializationInterface* a_serializer, std::uint32_t a_version)
	{
	...
	bool ILoadDelayFunctor::Save(const F4SE::SerializationInterface* a_serializer)
	{
		auto stream = Save::SaveStream(a_serializer);
